    ...
    <div class="container">
        ...
        <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="navbar-nav me-auto">
                <li class="nav-item">
                    <a class="nav-link {% if request.resolver_match.url_name == 'index' and request.resolver_match.app_name == 'home' %}active{% endif %}" 
                       href="{% url 'home:index' %}">
                        <i class="fas fa-home me-2"></i>Home
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link {% if request.resolver_match.app_name == 'devices' %}active{% endif %}" 
                       href="{% url 'devices:index' %}">
                        <i class="fas fa-server me-2"></i>Devices
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link {% if request.resolver_match.app_name == 'projects' %}active{% endif %}" 
                       href="{% url 'projects:index' %}">
                        <i class="fas fa-project-diagram me-2"></i>Projects
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link {% if request.resolver_match.app_name == 'chat' %}active{% endif %}" 
                       href="{% url 'chat:index' %}">
...
for app in home accounts devices chat projects; do
    ...
    # Create urls.py with conditional for accounts app
    if [ "$app" = "accounts" ]; then
...
EOF
        ...
        # Create extended views.py for accounts
...
EOF
        ...
        # Create additional templates for accounts
        for template in login signup profile; do
            cat > "${INSTALL_PATH}/src/${app}/templates/${app}/${template}.html" << EOF
{% extends 'base.html' %}
...
EOF
        ...
        # Default urls.py for other apps
        cat > "${INSTALL_PATH}/src/${app}/urls.py" << EOF
from django.urls import path
from . import views
...
EOF
        ...
        # Default views.py for other apps
        cat > "${INSTALL_PATH}/src/${app}/views.py" << EOF
...
EOF
    ...
    # Create default index.html template for all apps
    cat > "${INSTALL_PATH}/src/${app}/templates/${app}/index.html" << EOF
{% extends 'base.html' %}
    ...
    <div class="col-lg-6 mx-auto">
        ...
        <div class="d-grid gap-2 d-sm-flex justify-content-sm-center">
            {% if not user.is_authenticated %}
                <a href="{% url 'accounts:signup' %}" class="btn btn-primary btn-lg px-4 gap-3">Get Started</a>
            ...
            {% else %}
                ...
                <a href="{% url 'chat:index' %}" class="btn btn-outline-secondary btn-lg px-4">Chat</a>
            {% endif %}
        </div>
    </div>
</div>
