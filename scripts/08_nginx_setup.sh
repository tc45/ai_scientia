...
# Check if running in virtual environment and exit if so
if [ -n "$VIRTUAL_ENV" ]; then
    log_message "WARNING: Running in virtual environment. Deactivating..."
    deactivate
fi

# Check if nginx is installed
if ! command -v nginx &> /dev/null; then
    log_message "Nginx not found. Installing..."
    sudo apt-get update
...
server {
    ...
    location /static/ {
        alias $INSTALL_PATH/src/staticfiles/;
        expires 30d;
